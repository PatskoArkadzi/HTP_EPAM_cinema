...
import static by.htp.epam.cinema.web.util.constant.ResourceBundleKeysConstantDeclaration.ERROR_MSG_CHANGE_USER_PASSWORD_ACTION_INDEFINITE_ERROR;
import static by.htp.epam.cinema.web.util.constant.ResourceBundleKeysConstantDeclaration.ERROR_MSG_CHANGE_USER_PASSWORD_ACTION_INCORRECT_PASSWORD;
		...
		try {
			...
			if (user != null) {
				...
				request.setAttribute(REQUEST_PARAM_ERROR_MESSAGE,
						resourceManager.getValue(ERROR_MSG_CHANGE_USER_PASSWORD_ACTION_INCORRECT_PASSWORD));
				request.getRequestDispatcher(PAGE_ERROR).forward(request, response);
			}
		} catch (ValidateParamException e) {
			request.setAttribute(REQUEST_PARAM_ERROR_MESSAGE,
					resourceManager.getValue(ERROR_MSG_CHANGE_USER_PASSWORD_ACTION_INDEFINITE_ERROR));
			request.getRequestDispatcher(PAGE_ERROR).forward(request, response);
		}
	}
}
